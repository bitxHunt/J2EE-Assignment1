		...
		try {
			...
			if (e.getSQLState().equals("23503")) {
				...
				errors.put("deleteError",
						"This service cannot be deleted because it is currently part of active bundles or has existing schedules or transactions. As an alternative, you can set the service as inactive to hide it from customers.");

				HttpSession session = request.getSession();
				session.setAttribute("editServiceFormErrors", errors);
				response.sendRedirect(
						request.getContextPath() + "/admin/edit-service/" + request.getPathInfo().substring(1));
			} else {
