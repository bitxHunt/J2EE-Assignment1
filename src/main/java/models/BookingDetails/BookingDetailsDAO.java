...
public class BookingDetailsDAO {
	...
	public ArrayList<BookingDetails> getAllBookings(int page) throws SQLException {
		ArrayList<BookingDetails> bookings = new ArrayList<>();
		Connection conn = null;

		try {
			conn = DB.connect();
			String sql = "SELECT cb.id, cb.status, cb.date, cb.created_at, " + "ts.start_time, ts.end_time, "
					...
					+ "LEFT JOIN users staff ON sb.staff_id = staff.id " + "ORDER BY cb.created_at DESC "
					+ "LIMIT ? OFFSET ?";
